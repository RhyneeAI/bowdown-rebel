...
class PromotionService 
{
    public function create(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'nama_promosi' => 'required|string|max:30',
        ...
        return Promotion::create([
            'nama_promosi' => $validated['nama_promosi'],
            ...
            'foto' => $filename,
            'slug' => MakeSlug($validated['nama_promosi']),
    ...
    {
        $promotion = Promotion::select(['id', 'nama_promosi', 'kode_promosi','stok','diskon_harga','tanggal_mulai','tanggal_berakhir','foto','slug'])->orderBy('id', 'DESC')->get();
        return $promotion;
    }

    public function getAllWithPaginate(Int $paginate)
    {
        $promotion = Promotion::select(['id', 'nama_promosi', 'kode_promosi','stok','diskon_harga','tanggal_mulai','tanggal_berakhir','foto'])->orderBy('id', 'DESC')->paginate($paginate);
        return $promotion;
    }

    public function getOne(String $slug = '')
    {
        $promotion = Promotion::select(['id', 'nama_promosi', 'kode_promosi','stok','diskon_harga','tanggal_mulai','tanggal_berakhir','foto','slug'])->where('slug', $slug)->first();
        return $promotion;
    }
